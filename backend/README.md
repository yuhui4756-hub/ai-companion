...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\verify-release-candidate.ps1 -ExpectedVersion 0.1.3 -ExpectedSchemaVersion 5
```

`v0.1.3` 已公开发布；当时公开资产的 sidecar schema 为 v4。M3-B 之后当前源码 schema 为 v5，用于保留聊天引用 trace。准备后续新版本时需先同步 bump `package.json` 和 `package-lock.json`，再重新构建并用对应版本号核验。该核验脚本只读检查本地 `release-v06d/`，会用临时 SQLite 启动 packaged `suoyi-backend.exe` 检查 `/health` schema 和 `/db/status` 基础字段，不会上传 GitHub Release，也不会读取或打印 `GH_TOKEN`。
...
- 默认不调用 embedding。只有用户在知识面板单独开启向量检索并构建索引时，向量才参与 hybrid retrieval。本地 Ollama provider 只允许 `127.0.0.1/localhost` 地址，不需要 Key，也不会把资料发到外网；远程 OpenAI 兼容 provider 需要用户填写 embedding Key，active 知识切片和有明确检索线索的本次 query 才会发给用户配置的 embedding 服务商。Embedding Key 与聊天模型 Key 隔离，只保存在 renderer localStorage，不写入 SQLite、日志、导出或安装包。
