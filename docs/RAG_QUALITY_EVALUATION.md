...
- 类真实资料形态基准包含 12 份不敏感 Markdown 文档和 138 个问题，其中 118 个问题覆盖本地 BM25/关键词检索、泛字段澄清和无关问题，20 个问题覆盖 mock hybrid 语义改写。
...
- 当前扩展基准：12 份合成 Markdown 资料，138/138 通过，pass rate 为 100.0%；其中 lexical/FTS 为 118/118，hybrid/mock embedding 为 20/20。
- 推荐对外表述：构建 RAG 检索与 prompt 注入评测体系，将 48 题 pilot benchmark pass rate 从 77.1% 提升到 100.0%，并扩展到 138 题合成基准，覆盖字段问答、表格切片、无答案澄清和语义改写；删除后不召回由自动化回归单独覆盖。
