...
2026-08-03 补充 M3-A/M3-B 引用可解释性切片，目标是把 RAG 从“系统内部偷偷检索”变成用户和开发者都能看见的证据链。
...
- 配合现有评测脚本，形成“检索门、prompt 注入、最终回答、引用证据、持久化恢复”五层可观测闭环。

边界：

- 当前定位到 source 级卡片并显示片段编号；还没有做 chunk 原文展开、全文内高亮、PDF 页预览或 DOCX 原文件打开。
