...
- RAG-Q1 已完成结构化切片与本地检索质量基线：后端 schema v3 支持 Markdown 标题/列表/表格/问答/事实字段切片、FTS5/BM25 检索、关键词 fallback、低置信和泛字段问题不注入 prompt、旧库非破坏性迁移与元数据回填。远程 embedding/hybrid fusion 尚未完成。
...
- 继续完善 RAG/知识库能力：在 RAG-Q1 本地 FTS5/BM25 基线之上，设计远程 embedding、混合检索融合、召回解释和持续评测，同时保持用户明确开启和隐私外发边界。
