...
- `0.1.2` 已公开发布到 GitHub Release，Windows 安装包内置 Python sidecar、本地 SQLite 知识库和核心数据迁移；远端 `latest.yml` 已指向 `suoyi-setup-0.1.2.exe`。代码签名、SmartScreen 文案、真实用户自动更新端到端和回滚仍需后续验收持续记录。
...
- 当前正在准备 `v0.1.3` 本地候选资产，目标是把 RAG-Q1、RAG-H1 和发布候选 packaged sidecar schema smoke 增强打入 Windows 安装包候选；本轮不公开发布、不上传 GitHub Release、不触发真实自动更新。
