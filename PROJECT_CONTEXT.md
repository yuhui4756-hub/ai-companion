...
- 工程化 2D 已完成发布准备材料和本地候选资产核验脚本：可检查目标版本 installer、blockmap、`latest.yml`、packaged sidecar、禁入本地数据和常见密钥形态。
- `0.1.3` 已公开发布到 GitHub Release，Windows 安装包内置 Python sidecar、本地 SQLite 知识库、核心数据迁移、RAG-Q1 结构化检索、RAG-H1 远程 embedding 隐私门控与 hybrid retrieval；远端 `latest.yml` 已指向 `suoyi-setup-0.1.3.exe`。`0.1.2` Release 保留为手动回退下载点。`0.1.2 -> 0.1.3` 自动更新端到端已补验通过；代码签名、SmartScreen 文案和回滚仍需后续持续记录。
- RAG-Q1 已完成结构化切片与本地检索质量基线：后端 schema v3 支持 Markdown 标题/列表/表格/问答/事实字段切片、FTS5/BM25 检索、关键词 fallback、低置信和泛字段问题不注入 prompt、旧库非破坏性迁移与元数据回填。
- RAG-H1 已完成远程 embedding 隐私门控与 hybrid retrieval 最小闭环：后端 schema v4 支持去 Key 的 embedding 配置、SQLite JSON vector 索引、mock/OpenAI-compatible embeddings、索引状态/重建、BM25/关键词 + 向量融合检索、provider 失败脱敏降级；前端知识面板支持默认关闭、单独 Key、测试连接、重建索引和聊天检索运行时配置。真实远程 provider 端到端、高容量索引、命中解释 UI 和持续评测基准仍是后续项。
- `v0.1.3` 本地候选资产已通过 GitHub Actions 发布为公开 Release；发布 workflow 已在远端重新构建、运行后端测试、执行候选核验并上传 installer、blockmap 和 `latest.yml`。自动更新验收使用临时安装目录、真实 GitHub 更新源和测试 marker，确认升级后版本为 `0.1.3`、marker 保留、packaged sidecar schemaVersion 为 4；测试 marker 和临时安装记录已清理。
...
- 继续完善 RAG/知识库能力：在 RAG-H1 远程 embedding/hybrid 最小闭环之上，推进真实 provider 验收边界、索引任务 UX、向量隐私管理、召回解释和持续评测，同时保持用户明确开启和隐私外发边界。
