...
- `0.1.3` 已公开发布到 GitHub Release，Windows 安装包内置 Python sidecar、本地 SQLite 知识库、核心数据迁移、RAG-Q1 结构化检索、RAG-H1 远程 embedding 隐私门控与 hybrid retrieval；远端 `latest.yml` 已指向 `suoyi-setup-0.1.3.exe`。`0.1.2` Release 保留为手动回退下载点。代码签名、SmartScreen 文案、真实用户自动更新端到端和回滚仍需后续验收持续记录。
...
- `v0.1.3` 本地候选资产已通过 GitHub Actions 发布为公开 Release；发布 workflow 已在远端重新构建、运行后端测试、执行候选核验并上传 installer、blockmap 和 `latest.yml`。本次未在用户真实安装环境执行覆盖式自动更新验收。
