...
一个中文 BYOK（Bring Your Own Key，自带 API Key）AI 恋爱伴侣网页 Demo。当前以“单聊天页”为主体验：左侧只保留伴侣名字列表、新建和更多入口，中间专注聊天，顶部按钮打开伴侣、记忆、风格、设置和用户须知弹窗；旧朋友、理性支持、日常陪伴、角色陪伴类型默认收进更多/兼容入口。
...
如果提示 5173 端口已经被占用，通常说明已经有一个 AI伴侣窗口或服务在运行。可以直接打开 `http://127.0.0.1:5173/` 继续用，或者关闭旧命令窗口后再双击启动。
