...
- 下载地址：[suoyi-setup-0.1.0.exe](https://github.com/yuhui4756-hub/ai-companion/releases/download/v0.1.0/suoyi-setup-0.1.0.exe)
- Release 页面：[所依 v0.1.0](https://github.com/yuhui4756-hub/ai-companion/releases/tag/v0.1.0)
...
- 设置弹窗可清除本地 API Key、当前聊天、长期记忆、风格摘要，并导出不含 API Key 和原始聊天记录的配置/记忆 JSON。
- 预留 `channel-adapter`，第一版只实现 Web，QQ/微信为占位。

## Windows 桌面版：所依

v0.1.0 已发布 Windows 桌面安装包。用户可见应用名统一为“所依”，桌面窗口会隐藏系统默认 `File / Edit / View / Window / Help` 菜单栏，改用应用内自定义标题栏和最小化、最大化/还原、关闭按钮，并接入正式应用图标。

桌面版更新源使用公开 GitHub Releases。客户端不内置 GitHub token；发布 token 只能用于发布端环境变量或 CI Secret，不能写入客户端或配置文件。当前线上版本和安装包版本都是 `0.1.0`，因此不会提示有新版本；后续发布更高版本号时才会触发更新提示。
...
7. 点顶部“风格”可以导入或添加风格参考摘要，并绑定到当前伴侣。

## 隐私说明

当前应用没有后端服务器。API Key、聊天记录、长期记忆、伴侣配置和风格摘要默认保存在当前浏览器或桌面应用本地的 `localStorage`，项目不内置、不提交任何真实 Key。

聊天时，浏览器会用你填写的接口直接请求模型服务商。首次打开页面会显示用户须知弹窗，确认状态也只保存在当前浏览器；后续可从顶部“须知”按钮重新打开。
...
如果要换电脑、换浏览器，或从网页版迁移到桌面版，建议先在“设置”里导出配置/记忆作为备份，再在新环境中导入。导出文件不包含 API Key，也不默认包含原始聊天记录。

## 当前边界

- v0.1.0 已提供 Windows 桌面安装包和公开 GitHub Release，但当前没有代码签名证书。
- 当前没有后端账号系统，也没有云同步；数据默认保存在本地。
- QQ/微信接入仍是占位，不在当前版本范围内。
