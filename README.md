...
- 本地知识库：可在顶部“知识”入口手动粘贴文本或 Markdown；Python 后端 API 已支持导入 `.txt/.md/.pdf/.docx`，其中 PDF 仅支持文本层、DOCX 支持段落/列表/表格。启动 Python 本地后端后，资料会写入本机 SQLite、按 Markdown 标题/列表/表格/问答/事实字段结构切片，并通过本地 FTS5/BM25 或关键词 fallback 检索；相关且置信足够时把命中片段标记为“用户导入资料”后注入，泛字段或低置信问题不强行注入；删除资料后不再注入。向量检索默认关闭，支持本地 Ollama embedding 和远程 OpenAI 兼容 embedding，明确开启并重建索引后才参与 hybrid retrieval。
...
桌面版更新源使用公开 GitHub Releases。客户端不内置 GitHub token；发布 token 只能用于发布端环境变量或 CI Secret，不能写入客户端或配置文件。已完成 `0.1.2 -> 0.1.3` 自动更新端到端验收：旧版可发现 `0.1.3`，用户确认后下载并重启安装，升级后本地 marker 和随包 Python sidecar 均保留可用。
...
桌面开发态会尝试由 Electron 自动托管 `.venv` 里的 Python 后端：默认端口 `8765`，占用时尝试 `8766-8780`。桌面打包态会启动 `resources/python-backend/suoyi-backend.exe`。托管时 SQLite 文件会放到 Electron `userData` 下的 `backend/suoyi.sqlite`。`v0.1.3` 公开安装包已内置 schema v4 sidecar，用于向量检索和增强核验脚本。

如需本地向量检索，可先安装并启动 Ollama，然后拉取一个 embedding 模型：

```powershell
ollama pull bge-m3
```

`bge-m3` 约 1.2GB，更适合作为中文检索基线。若网络或磁盘压力较大，可以先用轻量备选：

```powershell
ollama pull nomic-embed-text
```

在“知识 -> 向量检索”里选择 `Ollama 本地` 或 `Ollama 轻量`，保存配置、测试连接并重建索引。本地 Ollama 模式只允许 `127.0.0.1/localhost` 地址，不需要 Embedding Key。
...
7. 点顶部“知识”可以手动粘贴你授权使用的文本或 Markdown；后端文件导入 API 已支持 `.txt/.md/.pdf/.docx` 文本层解析，完整 UI 文件上传入口留给后续小版本。相关提问时才会注入命中片段，删除后不再注入。
...
知识库资料默认只保存在本地。通过后端文件导入时，只保存解析出的文本、表格行和页码/章节等 metadata，默认不保存原始文件副本。聊天时，前端会把最新用户输入发给本机 Python 后端做本地检索；只有命中的相关片段会随当前模型请求发给你配置的模型服务商。删除资料会软删除对应来源和切片，后续不再注入。

当前 RAG 默认使用本地 FTS5/BM25 与关键词规则基线。向量检索是单独开关：本地 Ollama 模式不需要 Key，只调用本机 `127.0.0.1/localhost` 的 embedding 模型；远程 OpenAI 兼容模式需要用户明确填写 embedding Key，active 资料切片才会发给用户配置的 embedding 服务商。聊天检索时也只有在向量索引 ready 且问题有明确检索线索时，才会把本次 query 发给对应 embedding provider。Embedding Key 与聊天模型 Key 分开，默认只保存在当前环境本地 `localStorage`，不写入 SQLite、导出 JSON、日志或安装包。向量本身属于知识库资料的派生隐私数据，也不默认导出。
...
- 当前公开安装包已内置 Python sidecar、本地 SQLite 知识库、核心数据迁移、RAG-Q1 结构化切片/FTS5-BM25、RAG-H1/H2 embedding 隐私门控与 hybrid retrieval，并通过发布候选 sidecar schema smoke 增强降低旧后端误打包风险。后续仍需继续完善细粒度 CRUD、恢复/清理 UI、SQLite 加密或 OS 安全存储。
- RAG-H2 第一轮已加入可复现质量评测，覆盖相似资料不混答、口语字段、泛字段澄清、结构化切片和 mock hybrid 语义改写；RAG-M2-A 已加入后端 `.txt/.md/.pdf/.docx` 文本层解析与多格式检索基准，PDF 扫描件/OCR、图片和图表视觉理解仍未实现。详见 `docs/RAG_QUALITY_EVALUATION.md`。这不等于真实模型最终回答正确率已经完成，后续仍需用不敏感真实资料和问题集做端到端评估。
- 向量检索目前是轻量 hybrid retrieval：SQLite 保存 JSON 向量，Python 本地计算余弦相似度并与 BM25/关键词结果融合；支持本地 Ollama embedding 和远程 OpenAI 兼容 embedding，但没有引入远程向量库、云同步或重型本地向量数据库。
