...
- 本地知识库：可在顶部“知识”入口上传 `.txt/.md/.pdf/.docx` 文本层文件，或手动粘贴文本/Markdown；其中 PDF 仅支持文本层、DOCX 支持段落/列表/表格。启动 Python 本地后端后，资料会写入本机 SQLite、按 Markdown 标题/列表/表格/问答/事实字段结构切片，并通过本地 FTS5/BM25 或关键词 fallback 检索；相关且置信足够时把命中片段标记为“用户导入资料”后注入，泛字段或低置信问题不强行注入；删除资料后不再注入。向量检索默认关闭，支持本地 Ollama embedding 和远程 OpenAI 兼容 embedding，明确开启并重建索引后才参与 hybrid retrieval。
...
7. 点顶部“知识”可以上传你授权使用的 `.txt/.md/.pdf/.docx` 文本层文件，也可以继续手动粘贴文本或 Markdown。相关提问时才会注入命中片段，删除后不再注入。
...
- RAG-H2 第一轮已加入可复现质量评测，覆盖相似资料不混答、口语字段、泛字段澄清、结构化切片和 mock hybrid 语义改写；RAG-M2-A/M2-B 已加入 `.txt/.md/.pdf/.docx` 文本层解析、多格式检索基准和知识面板文件上传入口，PDF 扫描件/OCR、图片和图表视觉理解仍未实现。详见 `docs/RAG_QUALITY_EVALUATION.md`。这不等于真实模型最终回答正确率已经完成，后续仍需用不敏感真实资料和问题集做端到端评估。
