...
桌面开发态会尝试由 Electron 自动托管 `.venv` 里的 Python 后端：默认端口 `8765`，占用时尝试 `8766-8780`。托管时 SQLite 文件会放到 Electron `userData` 下的 `backend/suoyi.sqlite`。当前公开安装包仍是 `0.1.1`，尚未内置 Python 可执行文件和 backend 资源；本轮源码不发布新版安装包。
...
- Python 后端开发默认保存在 `backend/data/suoyi-dev.sqlite`，也可以用 `SUOYI_BACKEND_DB_PATH` 指定 SQLite 文件；Electron 托管时使用应用 `userData/backend/suoyi.sqlite`；`backend/data/` 和 `.sqlite/.db` 文件不会提交。
...
- 当前公开安装包还未内置 Python sidecar；Electron 源码开发态可自动托管本机 `.venv` 后端，公开发布需要后续单独确认。
