...
一个中文 BYOK（Bring Your Own Key，自带 API Key）AI 恋爱伴侣网页 Demo。当前 v0.4.1 以恋爱陪伴为主路径，支持男友/女友方向、主模板 + 融合气质、自定义恋爱人设 / system prompt、本地长期记忆分层管理、风格参考摘要和本地数据管理；旧朋友、理性支持、日常陪伴、角色陪伴类型默认收进更多/兼容入口。
...
2. 需要细调时，可以在创建流程或“伴侣”页编辑“恋爱人设 / system prompt”。这段只保存在当前浏览器本地，保存前会做本地边界检查。
...
4. 回到“聊天”开始对话。
5. 进入“伴侣”可以继续编辑当前恋爱伴侣，也可以从更多/兼容入口访问旧类型伴侣，或把旧伴侣恢复显示到主列表。
...
聊天时，浏览器会用你填写的接口直接请求模型服务商。首次打开页面会显示用户须知，确认状态也只保存在当前浏览器；后续可从侧栏或设置页重新打开。
