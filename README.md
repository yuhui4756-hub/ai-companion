...
v0.6-C 已提供 Electron 桌面壳骨架，用户可见应用名统一为“所依”。桌面窗口会隐藏系统默认 `File / Edit / View / Window / Help` 菜单栏，改用应用内自定义标题栏和最小化、最大化/还原、关闭按钮。

当前桌面版仍是 P0 骨架：暂不包含正式应用图标、代码签名、GitHub Releases 生产更新源或公开安装包发布物料。数据迁移仍通过“设置 -> 从网页版导入备份”显式完成，API Key 不在备份里，需要在桌面版重新填写。
