...
- 本地知识库：可在顶部“知识”入口手动导入文本或 Markdown；启动 Python 本地后端后，资料会写入本机 SQLite、按 Markdown 标题/列表/表格/问答/事实字段结构切片，并通过本地 FTS5/BM25 或关键词 fallback 检索；相关且置信足够时把命中片段标记为“用户导入资料”后注入，泛字段或低置信问题不强行注入；删除资料后不再注入。向量检索默认关闭，支持本地 Ollama embedding 和远程 OpenAI 兼容 embedding，明确开启并重建索引后才参与 hybrid retrieval。
