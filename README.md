...
v0.6-D 已提供 Electron 桌面壳骨架，用户可见应用名统一为“所依”。桌面窗口会隐藏系统默认 `File / Edit / View / Window / Help` 菜单栏，改用应用内自定义标题栏和最小化、最大化/还原、关闭按钮，并接入正式应用图标。

当前桌面版仍未配置代码签名，Windows 可能提示未知发布者。更新源准备使用公开 GitHub Releases；发布 token 只能用于发布端环境变量或 CI Secret，不能写入客户端或配置文件。数据迁移仍通过“设置 -> 从网页版导入备份”显式完成，API Key 不在备份里，需要在桌面版重新填写。
...
v0.6-A 先提供本地启动器方案，让它更像一个可以双击打开的本地软件。它还不是正式安装包，也没有引入 Electron/Tauri 桌面壳。
...
本项目的数据保存在“当前浏览器 + 当前访问地址”的 `localStorage` 里。为了让数据连续，本地启动统一使用：
...
如果以后要换电脑、换浏览器或迁移到真正桌面壳，建议先在“设置”里导出配置/记忆作为备份，再按后续迁移方案使用。导出文件不包含 API Key，也不默认包含原始聊天记录。

## 这还不是正式安装包

v0.6-A 的目标是先把现有 Web Demo 做成 Windows 上更容易启动的本地体验：双击脚本、固定地址、可构建预览、可回滚。

Electron / Tauri 这类真正桌面客户端会带来安装包、二进制下载、系统依赖、数据迁移和更新维护问题，后续阶段再评估。本阶段不引入后端、账号、云同步、QQ/微信或真实推送。
